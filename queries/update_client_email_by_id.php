...
if(isset($_SESSION['current_user_id'])){
    ...
    try {
        $sql = "UPDATE clients_email SET email='".str_replace('"', '\\"', str_replace("'", "\\'", $data['email']))."' WHERE email_id = " .$data['email_id'];
    ...
    // записываем в аудит
    $audit_event_type = "Изменение информации о e-mail клиента";
    $audit_event_data = "ID клиента: ".$tmp_client_id."\n";
    $audit_event_data = $audit_event_data."ФИО клиента: ".$tmp_fio."\n";
    $audit_event_data = $audit_event_data."Старый e-mail: ".$tmp_data."\n";
    $audit_event_data = $audit_event_data."Новый e-mail: ".$data['email'];
