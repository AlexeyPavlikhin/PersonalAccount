...
if(isset($_SESSION['current_user_id'])){
    ...
    $query = $connection->prepare(
        ...
        FROM 
            ...
            clients cls 
        WHERE 
            ph.telegram_id = ".$data['telegram_id']." 
    ...
    while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
         ...
         $tmp_fio = $row["fio"];
         $tmp_telegram = $row["telegram"];
    }    
    // получаем данные для записи в аудит (конец)

    try {
        $sql = "DELETE FROM clients_telegram WHERE telegram_id = " .$data['telegram_id'];
    ...
    // записываем в аудит
    $audit_event_type = "Удаление telegram клиента";
    $audit_event_data = "ID клиента: ".$tmp_client_id."\n";
    $audit_event_data = $audit_event_data."ФИО клиента: ".$tmp_fio."\n";
    $audit_event_data = $audit_event_data."Telegram: ".$tmp_telegram;
