...
if(isset($_SESSION['current_user_id'])){
    ...
    while ($row = $query->fetch(PDO::FETCH_ASSOC)) {
         ...
         $tmp_fio = $row["fio"];
         $tmp_data = $row["client_last_name"];
    }    
    // получаем данные для записи в аудит (конец)
    
    try {
        $sql = "UPDATE clients SET client_last_name='".str_replace('"', '\\"', str_replace("'", "\\'", $data['clientLastName']))."' WHERE client_id = ".$data['clientID'];
    ...
    // записываем в аудит
    $audit_event_type = "Изменение информации о фамилии клиента";
    $audit_event_data = "ID клиента: ".$tmp_client_id."\n";
    $audit_event_data = $audit_event_data."ФИО клиента: ".$tmp_fio."\n";
    $audit_event_data = $audit_event_data."Старая фамилия: ".$tmp_data."\n";
    $audit_event_data = $audit_event_data."Новая фамилия: ".$data['clientLastName'];
